...
def normalize_base_url(base_url: str) -> str:
    """Accept what people paste: add http:// when no scheme is given and drop
    a trailing slash."""
    url = base_url.strip()
...
def check_is_abs(base_url: str) -> None:
    ...
    try:
        ...
        raise AbsAuthError(f"couldn't reach an Audiobookshelf server at {base_url} ({reason})") from exc
    except (json.JSONDecodeError, UnicodeDecodeError) as exc:
        raise AbsAuthError(f"{base_url} answered, but it isn't an Audiobookshelf server") from exc
    if not (isinstance(data, dict) and data.get("success") is True):
        raise AbsAuthError(f"{base_url} answered, but it isn't an Audiobookshelf server")
...
def store_token(token: str) -> None:
    ...
    Raises AbsAuthError when no Secret Service provider (gnome-keyring,
    KeePassXC, ...) is available, which is common on minimal installs."""
    try:
        subprocess.run(
        ...
        raise AbsAuthError(NO_KEYRING_MESSAGE) from exc
    ...
    def _main():
        ...
        if command == "login":
            if len(sys.argv) < 4:
                ...
                sys.exit(1)
            base_url, username = normalize_base_url(sys.argv[2]), sys.argv[3]
            ...
            print(json.dumps({"ok": True, "baseUrl": base_url, "libraries": libraries,
